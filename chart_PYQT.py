...
class Window(QWidget):
    def __init__(self, parent=None):
        ...
        self.lbl_duration = QLabel('기간(예_20220512): ')
        self.lbl_t = QLabel('~')
        ...
        self.box_luu.addWidget(self.lbl_duration)
        ...
        for j,tickers in enumerate(ticker_group):
            for i,ticker in enumerate(tickers):
                if ticker: #리스트 값이 있으면
                    ...
                    line = lines[i]
                    print(ticker)
                    df = pd.read_sql(f"SELECT * FROM '{ticker}'", con).set_index('Date')
                    s = make_indicator.indicator(df,line)
...
class Chart(QWidget):
    ...
    # def __init__(self):
        ...
        crosshair.crosshair2(main_pg=p1_1, sub_pg1=p1_2, sub_pg2=p1_3,sub_pg3=p1_4,sub_pg4=p1_5,sub_pg5=p1_6,sub_pg6=p1_7,sub_pg7=p1_8,sub_pg8=p1_9,sub_pg9=p1_10,sub_pg10=p1_11,sub_pg11=p1_12)
def df_add(df,avg,ch_max):
    avgtime = int(avg)
    체결강도max = int(ch_max)
    체결강도min = 0

    df.drop(df.loc[df['open']<1].index, inplace=True) #시가가 1보다 작은 행은 결측으로 처리하여 삭제
    ...
    df['이평'] = df['close'].rolling(window=avgtime).mean()
    df['이평'] = df['이평'].fillna(np.nan)  # 이평 컬럼의 값이 nan일 경우 close로 대체
    df['이평'] = df['이평'].round(3)

    df['이평5'] = df['close'].rolling(window=5).mean()
    df['이평5'] = df['이평5'].round(3)
    # df['이평5'] = df['이평5'].fillna(np.nan)  # 이평 컬럼의 값이 nan일 경우 close로 대체
    df['이평60'] = df['close'].rolling(window=60).mean()
    df['이평60'] = df['이평60'].round(3)
    df['이평60마지'] = df['이평60']*0.98
    df['이평300'] = df['close'].rolling(window=300).mean()
    df['이평300'] = df['이평300'].round(3)
    df['이평300마지'] = df['이평300']*0.98
    df['이평1200'] = df['close'].rolling(window=1200).mean()
